  ...
  const headerStyle = task?.cover?.coverColor
    ...
    : task?.idAttachmentCover
      ? {
          ...
          backgroundColor: task?.cover?.edgeColor,
      ...
      <DialogContent className="task-edit">
        <header style={headerStyle} className="task-edit-header">
          ...
          <div className="task-header-actions">
            ...
            <Link
              to={`/board/${board?._id}`}
              className="icon-btn dynamic-btn link-btn task-header-btn"
            >
          ...
          <main>
            <section className="task-title task-grid-container">
              <div className="task-icon-check" onClick={completeTask}>
                {isChecked ? (
                  <span style={{ color: '#6A9A23' }} title="Mark incomplete">
                    <CircleCheckIcon width={16} height={16} fill="currentColor" />
                  </span>
                ) : (
                  <span title="Mark complete">
                    <CircleIcon width={16} height={16} fill="currentColor" />
                  </span>
                )}
              </div>
            ...
            <div className="task-content">
                  ...
                  {(task?.due || task?.start) && (
                    <section className="task-flex-container">
                      <h3 className="params-heading">
                        {task?.start && task?.due ? 'Dates' : task?.due ? 'Due date' : 'Start date'}
                      </h3>
                      <button
                        className="btn-neutral"
                        onClick={(event) => {
              ...
              <section className="task-grid-container task-description-container">
                ...
                {!task?.desc && !isDescEditing && (
                  <button className="editable" onClick={() => setIsDescEditing(true)}>
                    ...
                    <div key={comment}>
                      {!isThisCommentEditing && (
                        <div>
                          ...
                          <button
                            className="comment-btn"
                            onClick={() => onRemoveAction(comment._id)}
                          >
                            Delete
                          </button>
                        </div>
                      ...
                      {isThisCommentEditing && (
                        <form onSubmit={(ev) => onActionSubmit(ev, comment._id)}>
                          <textarea
                            ref={commentTextareaRef}
                            className="edit-textarea"
                            name="comment"
