...
export function CoverPicker({ task, onUpdateTask }) {
  ...
  const coverAttachments =
    task?.attachments?.filter((attachment) => attachment?.mimeType?.startsWith('image')) || []
      ...
      <div className="cover-editor">
        {(selectedAttachmentId || selectedColorKey) && (
          <div className="cover-preview">
            ...
            {selectedAttachmentId && (
              <img
                src={
                  coverAttachments.find((attachment) => attachment._id === selectedAttachmentId)
                    ?.url
                }
                alt=""
              />
        ...
        <ul className="cover-colors-container">
          {Object.keys(coverColorsMap).map((colorKey, idx) => (
            <li
              key={colorKey}
              className={`cover-colors-item ${selectedColorKey === colorKey ? 'selected' : ''}`}
            >
              <button
        ...
        <button className="btn-neutral" onClick={onRemoveCover}>
          Remove cover
        </button>
        ...
        {coverAttachments.length > 0 && (
          <div className="picker-attachments">
            {coverAttachments.length > 0 &&
              coverAttachments.map((attachment, idx) => {
          ...
          {(imgData, isUploading) => (
            <>
              {isUploading ? (
                ...
                <button className="btn-neutral upload-btn">Upload a cover image</button>
              )}
            </>
          )}
        </ImgUploader>
